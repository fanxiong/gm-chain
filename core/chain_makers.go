...
func GenerateChain(config *params.ChainConfig, parent *types.Block, engine consensus.Engine, db mitdb.Database, n int, gen func(int, *BlockGen)) ([]*types.Block, []types.Receipts) {
	if config == nil {
...
// header only chain.
func newCanonical(engine consensus.Engine, n int, full bool) (mitdb.Database, *BlockChain, error) {
	// Initialize a fresh chain with only a genesis block
	gspec := new(Genesis)
	db, _ := mitdb.NewMemDatabase()
	genesis := gspec.MustCommit(db)

	blockchain, _ := NewBlockChain(db, nil, params.AllMithashProtocolChanges, engine, vm.Config{})
...
func makeHeaderChain(parent *types.Header, n int, engine consensus.Engine, db mitdb.Database, seed int) []*types.Header {
...
func makeBlockChain(parent *types.Block, n int, engine consensus.Engine, db mitdb.Database, seed int) []*types.Block {
