...
var (
	...
	configPrefix   = []byte("fanxiong-config-") // config prefix for the db
...
func WriteCanonicalHash(db mitdb.Putter, hash common.Hash, number uint64) error {
...
func WriteHeadHeaderHash(db mitdb.Putter, hash common.Hash) error {
...
func WriteHeadBlockHash(db mitdb.Putter, hash common.Hash) error {
...
func WriteHeadFastBlockHash(db mitdb.Putter, hash common.Hash) error {
...
func WriteTrieSyncProgress(db mitdb.Putter, count uint64) error {
...
func WriteHeader(db mitdb.Putter, header *types.Header) error {
...
func WriteBody(db mitdb.Putter, hash common.Hash, number uint64, body *types.Body) error {
...
func WriteBodyRLP(db mitdb.Putter, hash common.Hash, number uint64, rlp rlp.RawValue) error {
...
func WriteTd(db mitdb.Putter, hash common.Hash, number uint64, td *big.Int) error {
...
func WriteBlock(db mitdb.Putter, block *types.Block) error {
...
func WriteBlockReceipts(db mitdb.Putter, hash common.Hash, number uint64, receipts types.Receipts) error {
...
func WriteTxLookupEntries(db mitdb.Putter, block *types.Block) error {
...
func WriteBloomBits(db mitdb.Putter, bit uint, section uint64, head common.Hash, bits []byte) {
	key := append(append(bloomBitsPrefix, make([]byte, 10)...), head.Bytes()...)
...
func PreimageTable(db mitdb.Database) mitdb.Database {
	return mitdb.NewTable(db, preimagePrefix)
}

// WritePreimages writes the provided set of preimages to the database. `number` is the
// current block number, and is used for debug messages only.
func WritePreimages(db mitdb.Database, number uint64, preimages map[common.Hash][]byte) error {
...
func WriteBlockChainVersion(db mitdb.Putter, vsn int) {
	enc, _ := rlp.EncodeToBytes(uint(vsn))
	db.Put([]byte("BlockchainVersion"), enc)
}

// WriteChainConfig writes the chain config settings to the database.
func WriteChainConfig(db mitdb.Putter, hash common.Hash, cfg *params.ChainConfig) error {
