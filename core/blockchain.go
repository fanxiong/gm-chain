...
type BlockChain struct {
	...
	db     mitdb.Database // Low level persistent database to store final content in
...
// available in the database. It initialises the default gm-chain Validator and
// Processor.
func NewBlockChain(db mitdb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config) (*BlockChain, error) {
	if cacheConfig == nil {
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
	...
	for i, block := range blockChain {
		...
		if batch.ValueSize() >= mitdb.IdealBatchSize {
...
func (bc *BlockChain) GetBalance(root common.Hash,addr common.Address) *big.Int {
	...
	if error !=nil{
		fmt.Println("create state error！")
		return common.Big0
	}
