...
func benchInsertChain(b *testing.B, disk bool, gen func(int, *BlockGen)) {
	...
	if !disk {
		...
		db, err = mitdb.NewLDBDatabase(dir, 128, 128)
		if err != nil {
	...
	chain, _ := GenerateChain(gspec.Config, genesis, mithash.NewFaker(), db, b.N, gen)

	// Time the insertion of the new chain.
	// State and blocks are stored in the same DB.
	chainman, _ := NewBlockChain(db, nil, gspec.Config, mithash.NewFaker(), vm.Config{})
	defer chainman.Stop()
	b.ReportAllocs()
	b.ResetTimer()
...
// into a database.
func makeChainForBench(db mitdb.Database, full bool, count uint64) {
...
func benchWriteChain(b *testing.B, full bool, count uint64) {
	for i := 0; i < b.N; i++ {
		...
		db, err := mitdb.NewLDBDatabase(dir, 128, 1024)
		if err != nil {
...
func benchReadChain(b *testing.B, full bool, count uint64) {
	...
	for i := 0; i < b.N; i++ {
		...
		chain, err := NewBlockChain(db, nil, params.TestChainConfig, mithash.NewFaker(), vm.Config{})
		if err != nil {
