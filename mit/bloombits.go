...
const (
	// bloomServiceThreads is the number of goroutines used globally by an Mit
...
func (eth *Mitereum) startBloomHandlers() {
...
// for the gm-chain header bloom filters, permitting blazing fast filtering.
type BloomIndexer struct {
	size uint64 // section size to generate bloombits for

	db  mitdb.Database       // database instance to write index data and metadata into
...
func NewBloomIndexer(db mitdb.Database, size uint64) *core.ChainIndexer {
	backend := &BloomIndexer{
		db:   db,
		size: size,
	}
	table := mitdb.NewTable(db, string(core.BloomBitsIndexPrefix))
