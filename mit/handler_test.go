...
func testGetNodeData(t *testing.T, protocol int) {
	...
	statedb, _ := mitdb.NewMemDatabase()
...
// Tests that post mit protocol handshake, DAO fork-enabled clients also execute
...
func testDAOChallenge(t *testing.T, localForked, remoteForked bool, timeout bool) {
	...
	if !timeout {
		blocks, _ := core.GenerateChain(&params.ChainConfig{}, genesis, mithash.NewFaker(), db, 1, func(i int, block *core.BlockGen) {
			if remoteForked {
