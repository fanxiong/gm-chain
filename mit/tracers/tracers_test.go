...
import (
	...
	"github.com/fanxiong/gm-chain/tests"
)

// To generate a new callTracer test, copy paste the makeTest method below into
// a MIT console and call it with a transaction hash you which to export.
...
var makeTest = function(tx, rewind) {
  ...
  var block   = mit.getBlock(mit.getTransaction(tx).blockHash);
  var genesis = mit.getBlock(block.parentHash);
  ...
  genesis.config = admin.nodeInfo.protocols.mit.config;
  ...
  console.log(JSON.stringify({
    ...
    input:  mit.getRawTransaction(tx),
    result: result,
  }, null, 2));
}
*/
