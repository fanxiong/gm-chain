...
import (
	...
	"github.com/fanxiong/gm-chain/mit/fetcher"
...
// NewProtocolManager returns a new Mit sub protocol manager. The Mit sub protocol manages peers capable
// with the Mit network.
func NewProtocolManager(config *params.ChainConfig, mode downloader.SyncMode, networkId uint64, mux *event.TypeMux, txpool txPool, engine consensus.Engine, blockchain *core.BlockChain, chaindb mitdb.Database) (*ProtocolManager, error) {
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing Mit peer", "peer", id)

	// Unregister the peer from the downloader and Mit peer set
...
func (pm *ProtocolManager) Stop() {
	log.Info("Stopping Mit protocol")
...
// handle is the callback invoked to manage the life cycle of an mit peer. When
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Mit peer connected", "name", p.Name())

	// Execute the Mit handshake
	var (
	...
	if err := p.Handshake(pm.networkId, td, hash, genesis.Hash()); err != nil {
		p.Log().Debug("Mit handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Mit peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Mit message handling failed", "err", err)
			return err
		}
	}
}
