...
type Database struct {
	diskdb mitdb.Database // Persistent storage for matured trie nodes
...
func NewDatabase(diskdb mitdb.Database) *Database {
	return &Database{
		diskdb: diskdb,
...
func (db *Database) commit(hash common.Hash, batch mitdb.Batch) error {
	// If the node does not exist, it's a previously committed node
