...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.fanxiong", new NodeConfig());
			node.start();
			...
			MitClient ec = node.getMitClient();
	...
	// signatures too. Regression test for gm-chain issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/fanxiong/gethtest", "libs"} {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.fanxiong", "github.com/fanxiong/gm-chain/mobile")
	...
	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "fanxiong", "gethtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
