...
// use snappy to compress the db,so the db can be small----change
...
// Put puts the given key / value to the queue // use snappy
func (db *LDBDatabase) Put(key []byte, value []byte) error {
	//snappyByte:=snappy.Encode(nil,value)
...
// Get returns the given key if it's present.// use snappy
func (db *LDBDatabase) Get(key []byte) ([]byte, error) {
	dat, err := db.db.Get(key, nil)
	if err != nil {
		return nil, err
	}
	//traceBuf,_:=snappy.Decode(nil,dat)
	return dat, nil
}
...
func (b *ldbBatch) Put(key, value []byte) error {
	//valueByte:=snappy.Encode(nil,[]byte(value))
	b.b.Put(key, value)
	b.size += len(value)
	return nil
}
