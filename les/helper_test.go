...
import (
	...
	"github.com/fanxiong/gm-chain/les/flowcontrol"
...
func newTestProtocolManager(lightSync bool, blocks int, generator func(int, *core.BlockGen), peers *peerSet, odr *LesOdr, db mitdb.Database) (*ProtocolManager, error) {
	var (
	...
	if lightSync {
		...
		bloomIndexer := mit.NewBloomIndexer(db, params.BloomBitsBlocks)
		bloomIndexer.AddChildIndexer(bbtIndexer)
		bloomIndexer.Start(blockchain)

		gchain, _ := core.GenerateChain(gspec.Config, genesis, mithash.NewFaker(), db, blocks, generator)
...
// fails the test.
func newTestProtocolManagerMust(t *testing.T, lightSync bool, blocks int, generator func(int, *core.BlockGen), peers *peerSet, odr *LesOdr, db mitdb.Database) *ProtocolManager {
