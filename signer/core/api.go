...
type ExternalAPI interface {
	...
	SignTransaction(ctx context.Context, args SendTxArgs, methodSelector *string) (*mitapi.SignTransactionResult, error)
...
type SignerUI interface {
	...
	OnApprovedTx(tx mitapi.SignTransactionResult)
...
func (api *SignerAPI) SignTransaction(ctx context.Context, args SendTxArgs, methodSelector *string) (*mitapi.SignTransactionResult, error) {
	var (
		err    error
	...
	response := mitapi.SignTransactionResult{Raw: rlpdata, Tx: signedTx}
...
// Sign calculates an Mit ECDSA signature for:
// keccack256("\x19Mit Signed Message:\n" + len(message) + message))
//
...
// the address of:
// hash = keccak256("\x19Mit Signed Message:\n"${message length}${message})
...
func (api *SignerAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	...
	if sig[64] != 27 && sig[64] != 28 {
		return common.Address{}, fmt.Errorf("invalid Mit signature (V is not 27 or 28)")
	}
...
//   keccak256("\x19Mit Signed Message:\n"${message length}${message}).
//
// This gives context to the signed message and prevents signing of transactions.
func SignHash(data []byte) ([]byte, string) {
	msg := fmt.Sprintf("\x19Mit Signed Message:\n%d%s", len(data), data)
