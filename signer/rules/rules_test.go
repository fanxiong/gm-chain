...
import (
	...
	"github.com/fanxiong/gm-chain/signer/core"
	"github.com/fanxiong/gm-chain/signer/storage"
)
...
func (alwaysDenyUI) OnApprovedTx(tx mitapi.SignTransactionResult) {
	panic("implement me")
}
...
func (d *dummyUI) OnApprovedTx(tx mitapi.SignTransactionResult) {
...
func TestForwarding(t *testing.T) {
	...
	r.OnApprovedTx(mitapi.SignTransactionResult{})
...
func TestLimitWindow(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		response := mitapi.SignTransactionResult{
			Tx:  dummySigned(v),
...
func (d *dontCallMe) OnApprovedTx(tx mitapi.SignTransactionResult) {
	d.t.Fatalf("Did not expect next-handler to be called")
}
