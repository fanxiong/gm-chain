...
web3._extend({
	...
	methods: [
		...
		new web3._extend.Method({
			...
			call: 'mit_submitTransaction',
			params: 1,
			inputFormatter: [web3._extend.formatters.inputTransactionFormatter]
		}),
		new web3._extend.Method({
			name: 'getRawTransaction',
			call: 'mit_getRawTransactionByHash',
			params: 1
		}),
		new web3._extend.Method({
			name: 'getRawTransactionFromBlock',
			call: function(args) {
				return (web3._extend.utils.isString(args[0]) && args[0].indexOf('0x') === 0) ? 'mit_getRawTransactionByBlockHashAndIndex' : 'mit_getRawTransactionByBlockNumberAndIndex';
			},
			params: 2,
	...
	properties: [
		new web3._extend.Property({
			...
			getter: 'mit_pendingTransactions',
