...
func (mithash *Mithash) VerifyHeader(chain consensus.ChainReader, header *types.Header, seal bool) error {
	...
	return mithash.verifyHeader(chain, header, parent, false, seal)
}
...
func (mithash *Mithash) VerifyHeaders(chain consensus.ChainReader, headers []*types.Header, seals []bool) (chan<- struct{}, <-chan error) {
	// If we're running a full engine faking, accept any input as valid
	if mithash.config.PowMode == ModeFullFake || len(headers) == 0 {
	...
	for i := 0; i < workers; i++ {
		go func() {
			for index := range inputs {
				errors[index] = mithash.verifyHeaderWorker(chain, headers, seals, index)
				done <- index
			}
		}()
	}
...
func (mithash *Mithash) verifyHeaderWorker(chain consensus.ChainReader, headers []*types.Header, seals []bool, index int) error {
	...
	return mithash.verifyHeader(chain, headers[index], parent, false, seals[index])
}
...
func (mithash *Mithash) VerifyUncles(chain consensus.ChainReader, block *types.Block) error {
	...
	for _, uncle := range block.Uncles() {
		...
		if err := mithash.verifyHeader(chain, uncle, ancestors[uncle.ParentHash], true, true); err != nil {
			return err
		}
	}
	return nil
...
func (mithash *Mithash) verifyHeader(chain consensus.ChainReader, header, parent *types.Header, uncle bool, seal bool) error {
	...
	expected := mithash.CalcDifficulty(chain, header.Time.Uint64(), parent)
...
func (mithash *Mithash) VerifySeal(chain consensus.ChainReader, header *types.Header) error {
	...
	if mithash.shared != nil {
		return mithash.shared.VerifySeal(chain, header)
	}
...
// header to conform to the mithash protocol. The changes are done inline.
func (mithash *Mithash) Prepare(chain consensus.ChainReader, header *types.Header) error {
	parent := chain.GetHeader(header.ParentHash, header.Number.Uint64()-1)
	if parent == nil {
		return consensus.ErrUnknownAncestor
	}
	header.Difficulty = mithash.CalcDifficulty(chain, header.Time.Uint64(), parent)
	return nil
}
