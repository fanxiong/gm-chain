...
func initSwarmNode(config *bzzapi.Config, stack *node.Node, ctx *cli.Context) {
	...
	//set the resolved config path (mit --datadir)
...
func envVarsOverride(currentConfig *bzzapi.Config) (config *bzzapi.Config) {
	...
	if datadir := os.Getenv(GMIT_ENV_DATADIR); datadir != "" {
		currentConfig.Path = datadir
	}
...
func checkDeprecated(ctx *cli.Context) {
	// exit if the deprecated --mitapi flag is set
	if ctx.GlobalString(DeprecatedEthAPIFlag.Name) != "" {
		utils.Fatalf("--mitapi is no longer a valid command line flag, please use --ens-api and/or --swap-api.")
	}
