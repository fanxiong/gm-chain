Clef
----
Clef can be used to sign transactions and data and is meant as a replacement for mit's account management.
This allows DApps not to depend on mit's account management. When a DApp wants to sign data it can send the data to
...
GLOBAL OPTIONS:
   ...
   --keystore value        Directory for the keystore (default: "$HOME/.fanxiong/keystore")
...
The general flow for signing a transaction using e.g. mit is as follows:
![image](sign_flow.png)

In this case, `mit` would be started with `--externalsigner=http://localhost:8550` and would relay requests to `eth.sendTransaction`.
...
put together is a bit of a hack into the http server. This could probably be greatly improved
    - Relay: Gmit should be started in `mit --external_signer localhost:8550`.
    - Currently, the Gmit APIs use `common.Address` in the arguments to transaction submission (e.g `to` field). This
  ...
  retains the original input.
    - The Gmit api should switch to use the same type, and relay `to`-account verbatim to the external api.
...
The signer listens to HTTP requests on `rpcaddr`:`rpcport`, with the same JSONRPC standard as Gmit. The messages are
...
The signer will generate a new private key, encrypts it according to [web3 keystore spec](https://github.com/fanxiong/wiki/wiki/Web3-Secret-Storage-Definition) and stores it in the keystore directory.
...
#### Arguments
  - account [object]: key in [web3 keystore format](https://github.com/fanxiong/wiki/wiki/Web3-Secret-Storage-Definition) (retrieved with account_export)
...
#### Result
  - exported key, see [web3 keystore format](https://github.com/fanxiong/wiki/wiki/Web3-Secret-Storage-Definition) for
  more information
  ...
  "params": [
    {
      "accounts": [
        {
          "type": "Account",
          "url": "keystore:///home/bazonk/.fanxiong/keystore/UTC--2017-11-20T14-44-54.089682944Z--123409812340981234098123409812deadbeef42",
          "address": "0x123409812340981234098123409812deadbeef42"
        },
        {
          "type": "Account",
          "url": "keystore:///home/bazonk/.fanxiong/keystore/UTC--2017-11-23T21-59-03.199240693Z--cafebabedeadbeef34098123409812deadbeef42",
...
* A Graphical UI MUST show the blocky-identicon for fanxiong addresses.
