...
type mitConfig struct {
	...
	Shh       whisper.Config
	Node      node.Config
	Mitstats  mitstatsConfig
	Dashboard dashboard.Config
}

func loadConfig(file string, cfg *mitConfig) error {
	f, err := os.Open(file)
	if err != nil {
		return err
	}
...
func defaultNodeConfig() node.Config {
	...
	cfg.HTTPModules = append(cfg.HTTPModules, "mit", "shh")
	cfg.WSModules = append(cfg.WSModules, "mit", "shh")
	cfg.IPCPath = "mit.ipc"
	return cfg
}

func makeConfigNode(ctx *cli.Context) (*node.Node, mitConfig) {
	// Load defaults.
	cfg := mitConfig{
		Mit:       mit.DefaultConfig,
	...
	utils.SetMitConfig(ctx, stack, &cfg.Mit)
	if ctx.GlobalIsSet(utils.MitStatsURLFlag.Name) {
		cfg.Mitstats.URL = ctx.GlobalString(utils.MitStatsURLFlag.Name)
	}
...
func makeFullNode(ctx *cli.Context) *node.Node {
	...
	utils.RegisterMitService(stack, &cfg.Mit)
