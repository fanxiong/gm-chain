...
func TestAccountListEmpty(t *testing.T) {
	mit := runGmit(t, "account", "list")
	mit.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	mit := runGmit(t, "account", "list", "--datadir", datadir)
	defer mit.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	mit := runGmit(t, "account", "new", "--lightkdf")
	defer mit.ExpectExit()
	mit.Expect(`
...
`)
	mit.ExpectRegexp(`Address: \{[0-9a-f]{40}\}\n`)
}
...
func TestWalletImport(t *testing.T) {
	mit := runGmit(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer mit.ExpectExit()
	mit.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(mit.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(mit.StderrText(), m) {
...
// https://github.com/fanxiong/gm-chain/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	mit.SetTemplateFunc("keypath", func(file string) string {
