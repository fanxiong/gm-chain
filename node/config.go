...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of gmit is "gmit". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Mit", keep that.
	if name == "mit" || name == "mit-testnet" {
		name = "Mit"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "mit" instances.
var isOldGmitResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) resolvePath(path string) string {
	...
	if c.name() == "mit" && isOldGmitResource[path] {
		oldpath := ""
		if c.Name == "mit" {
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "gm-chain-keystore")
		ephemeral = keydir
	}
