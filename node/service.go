...
func (ctx *ServiceContext) OpenDatabase(name string, cache int, handles int) (mitdb.Database, error) {
	if ctx.config.DataDir == "" {
		return mitdb.NewMemDatabase()
	}
	db, err := mitdb.NewLDBDatabase(ctx.config.resolvePath(name), cache, handles)
	if err != nil {
		return nil, err
	}
	return db, nil
