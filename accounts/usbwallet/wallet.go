...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the Mit
...
type wallet struct {
	...
	deriveChain    fanxiong.ChainStateReader // Blockchain state reader to discover used account with
...
func (w *wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		for empty := false; !empty; {
			// Retrieve the next derived gm-chain account
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain fanxiong.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
