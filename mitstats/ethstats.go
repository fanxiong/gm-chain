...
// Service implements an Mit netstats reporting daemon that pushes local
// chain statistics up to a monitoring server.
type Service struct {
	server *p2p.Server        // Peer-to-peer server to retrieve networking infos
	eth    *mit.Mitereum      // Full Mit service if monitoring a full node
	les    *les.LightMit // Light Mit service if monitoring a light node
...
func New(url string, ethServ *mit.Mitereum, lesServ *les.LightMit) (*Service, error) {
...
func (s *Service) login(conn *websocket.Conn) error {
	...
	if info := infos.Protocols["mit"]; info != nil {
		network = fmt.Sprintf("%d", info.(*mit.NodeInfo).Network)
		protocol = fmt.Sprintf("mit/%d", mit.ProtocolVersions[0])
	} else {
...
func (s *Service) reportLatency(conn *websocket.Conn) error {
	// Send the current time to the mitstats server
	start := time.Now()
	...
	log.Trace("Sending measured latency to mitstats", "latency", latency)
...
func (s *Service) reportBlock(conn *websocket.Conn, block *types.Block) error {
	...
	log.Trace("Sending new block to mitstats", "number", details.Number, "hash", details.Hash)
...
func (s *Service) reportHistory(conn *websocket.Conn, list []uint64) error {
	...
	if len(history) > 0 {
		log.Trace("Sending historical blocks to mitstats", "first", history[0].Number, "last", history[len(history)-1].Number)
	} else {
...
func (s *Service) reportPending(conn *websocket.Conn) error {
	...
	log.Trace("Sending pending transactions to mitstats", "count", pending)
...
func (s *Service) reportStats(conn *websocket.Conn) error {
	...
	log.Trace("Sending node details to mitstats")
