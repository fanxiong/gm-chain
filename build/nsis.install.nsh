Name "mit ${MAJORVERSION}.${MINORVERSION}.${BUILDVERSION}" # VERSION variables set through command line arguments
InstallDir "$InstDir"
...
# Install mit binary
Section "MIT" GMIT_IDX
  SetOutPath $INSTDIR
  file {{.MIT}}

  # Create start menu launcher
  createDirectory "$SMPROGRAMS\${APPNAME}"
  createShortCut "$SMPROGRAMS\${APPNAME}\${APPNAME}.lnk" "$INSTDIR\gmit.exe" "--fast" "--cache=512"
  createShortCut "$SMPROGRAMS\${APPNAME}\Attach.lnk" "$INSTDIR\gmit.exe" "attach" "" ""
...
Function GetInstalledSize
  ...
  ${if} ${SectionIsSelected} ${GMIT_IDX}
    SectionGetSize ${GMIT_IDX} $0
