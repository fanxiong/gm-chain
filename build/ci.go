...
var (
	// Files that end up in the gmit*.zip archive.
	gmitArchiveFiles = []string{
		"COPYING",
		executablePath("gmit"),
	}

	// Files that end up in the gmit-alltools*.zip archive.
...
func doArchive(cmdline []string) {
	...
	var (
		...
		gmit     = "gmit-" + base + ext
		alltools = "gmit-alltools-" + base + ext
	)
	maybeSkipArchive(env)
	if err := build.WriteArchive(gmit, gmitArchiveFiles); err != nil {
		log.Fatal(err)
	}
	if err := build.WriteArchive(alltools, allToolsArchiveFiles); err != nil {
		log.Fatal(err)
	}
	for _, archive := range []string{gmit, alltools} {
...
func makeWorkdir(wdflag string) string {
	...
	if wdflag != "" {
		...
		wdflag, err = ioutil.TempDir("", "gmit-build-")
	}
	if err != nil {
		log.Fatal(err)
	}
...
func doWindowsInstaller(cmdline []string) {
	...
	*workdir = makeWorkdir(*workdir)
	...
	// first section contains the mit binary, second section holds the dev tools.
	templateData := map[string]interface{}{
		"License":  "COPYING",
		"Gmit":     gmitTool,
		"DevTools": devTools,
	}
	build.Render("build/nsis.gmit.nsi", filepath.Join(*workdir, "gmit.nsi"), 0644, nil)
	...
	installer, _ := filepath.Abs("gmit-" + archiveBasename(*arch, env) + ".exe")
	build.MustRunCommand("makensis.exe",
		...
		"/DARCH="+*arch,
		filepath.Join(*workdir, "gmit.nsi"),
	)
...
func doAndroidArchive(cmdline []string) {
	...
	if *local {
		...
		os.Rename("gmit.aar", filepath.Join(GOBIN, "gmit.aar"))
		return
	}
	...
	archive := "gmit-" + archiveBasename("android", env) + ".aar"
	os.Rename("gmit.aar", archive)
...
func doXCodeFramework(cmdline []string) {
	...
	archive := "gmit-" + archiveBasename("ios", env)
	...
	if *deploy != "" {
		...
		build.Render("build/pod.podspec", "Gmit.podspec", 0755, meta)
		build.MustRunCommand("pod", *deploy, "push", "Gmit.podspec", "--allow-warnings", "--verbose")
	}
}
